...
.args <- if (interactive()) c(
  file.path("sim_pop", c(
    "locations.txt", "public-locations.txt", "population.txt", "network.txt"
  )),
  file.path("supp_locs_by_type.png"),
  file.path("supp_home_size_distr.png"),
  file.path("supp_work_size_distr.png"),
  file.path("supp_home_degree_distr.png")
) else commandArgs(trailingOnly = TRUE)

locs.dt = fread(.args[1], fill = TRUE)[,
  patronized := fifelse(locid %in% fread(.args[2])[,V1], 1, 0)
]
