...
.args <- if (interactive()) c(
  file.path("fig", c("digest-ext.rds", "digest-ext-key.rds", "digest-ref.rds", "vis_support.rda")),
  file.path("fig", "active-only.png")
...
p.ave <- p.gen(
  plt.dt, "averted", aesbase, "Test", scale_y_averted
) + theme(legend.position = "none")

p.inc = p.gen(
  setkeyv(rbind(plt.dt, ref.dt, fill = TRUE), key(plt.dt)),
