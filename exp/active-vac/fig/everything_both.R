...
sero.dt <- prepare(d[, .(realization, date, seroprev, season) ], id.vars = c("realization", "date", "season"))
...
p.sero <- p.core(
  ...
  data = seroprev
    ...
    y = est, shape="observation", color = NULL
  ), data = seroprev) +
  scale_y_fraction() +
  conserved +
  theme(
...
inc.dt <- prepare(
  ...
  id.vars = c("realization", "date", "season")
...
hinc.dt <- prepare(
  ...
  hhsHosp, id.vars = c("realization", "date", "season")
)[date < "2022-04-01"][!is.na(value)]
...
brk.dt <- rbind(prepare(
  ...
  data.table(date = as.Date("2020-02-01"), measure = "brkthru"),
  fill = TRUE
)
...
sd.dt <- d[realization == 1, .(date, value = sd, closed, season) ]
...
p.sd <- p.core(
  ...
  geom_text(
    aes(
      y = 0.175, x = start+(end-start)/2, label = "Lockdown", color = NULL
    ),
    data = function (dt) d[closed == 1, {
      spn = range(date)
      .(start = spn[1], end = spn[2])
...
vax.dt <- prepare(
  d[realization == 1 & season == TRUE, .(realization, date, cov1, cov2, cov3)],
